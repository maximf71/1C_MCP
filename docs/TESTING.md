...
Для версии 0.7.0 дополнительно проверьте сценарии из раздела «Приёмочная проверка» в [TOOLS_0_7.md](TOOLS_0_7.md). `go test -race ./...` требует C-компилятор для CGO; на Windows без GCC используйте повторный прогон `go test -count=5 ./...`.
...
Ожидается успешное завершение всех пакетов и строка `mcp-1c-analog 0.7.0`.
